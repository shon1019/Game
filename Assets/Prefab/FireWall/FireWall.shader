Shader "Custom/FireWall"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				fixed4 col = tex2D(_MainTex, i.uv * sin(_Time.x*0.3f + 0.5f)); // sample the texture
				fixed4 col2 = tex2D(_MainTex, i.uv * cos(_Time.x*0.4f + 0.5f));
				//UNITY_APPLY_FOG(i.fogCoord, col);   // apply fog
				return col2 * col * _Color;                // return the texture color * the color property
			}
			ENDCG
		}
	}
